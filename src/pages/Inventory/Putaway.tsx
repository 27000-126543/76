...
import type { PutawayTask, PutawayTaskStatus, ApprovalRecord } from '@/types';
...
const approvalLevelLabels: Record<number, string> = {
  1: '组长审批',
  2: '经理审批',
  3: '总监审批',
};
...
export default function InventoryPutawayPage() {
  ...
  const openDetail = (task: PutawayTask) => {
    setSelectedTask(task);
  ...
  const canAccept = (task: PutawayTask) => task.status === 'pending' && canAccess('leader');
  ...
  const renderApprovalTimeline = (approvals?: ApprovalRecord[]) => {
    if (!approvals || approvals.length === 0) {
      return <p className="text-sm text-dark-500">暂无审批记录</p>;
    }
    return (
          ...
          <div key={idx} className="flex items-start gap-3">
            <div
              className={cn(
                ...
                ap.action === 'approve' ? 'bg-success/20 text-success' : 'bg-danger/20 text-danger'
              )}
            >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <p className="text-sm font-medium text-white">
                  {approvalLevelLabels[ap.level] || `第${ap.level}级审批`}
                ...
                <p className="text-xs text-dark-500">{formatDateTime(ap.approvedAt)}</p>
              </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-5">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-3 mb-3">
                      ...
                      {hasSource && (
                        <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-md bg-accent-500/15 text-accent-500 border border-accent-500/30">
                          <FileText className="w-3 h-3" />
                          来源: {task.sourceRequestNo}
                        </span>
                      )}
                    </div>
                    ...
                    {hasSource && (
                      <div className="mb-3 rounded-lg bg-dark-900/50 border border-dark-700 px-4 py-3">
                        <div className="flex items-center gap-2 mb-2">
                          <FileText className="w-3.5 h-3.5 text-accent-500" />
                          <span className="text-xs font-medium text-dark-300">补货来源</span>
                        </div>
                        <div className="grid grid-cols-3 gap-4 text-xs">
                          <div>
                            <span className="text-dark-500">申请单号：</span>
                            <span className="text-dark-200 font-mono">{task.sourceRequestNo}</span>
                          </div>
                          <div>
                            <span className="text-dark-500">申请人：</span>
                            <span className="text-dark-200">{task.sourceApplicantName}</span>
                          </div>
                          {task.sourceApprovedAt && (
                            <div>
                              <span className="text-dark-500">审批通过：</span>
                              <span className="text-dark-200">{formatDateTime(task.sourceApprovedAt)}</span>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row lg:flex-col gap-3 lg:w-80 shrink-0">
                    ...
                    <div className="flex sm:items-end lg:justify-end gap-2 flex-wrap">
                      {canAccept(task) && (
                        <Button
                          variant="primary"
                          size="sm"
        ...
        {selectedTask && (
          <div className="space-y-5">
            <div className="rounded-lg bg-dark-900/50 border border-dark-700 p-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="text-dark-200 font-mono">{selectedTask.sourceRequestNo || '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-dark-500 mb-1">申请人</div>
                  <div className="text-dark-200">{selectedTask.sourceApplicantName || '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-dark-500 mb-1">商品</div>
                  <div className="text-white font-medium">
                    {selectedTask.skuName} <span className="text-dark-500 font-mono text-xs">({selectedTask.skuId})</span>
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="text-accent-500 font-mono font-semibold">{selectedTask.targetLocation}</div>
                </div>
                <div>
                  <div className="text-xs text-dark-500 mb-1">审批通过时间</div>
                  <div className="text-dark-200">
                    {selectedTask.sourceApprovedAt ? formatDateTime(selectedTask.sourceApprovedAt) : '-'}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              {renderApprovalTimeline(selectedTask.sourceApprovals)}
            </div>
          </div>
        )}
      </Modal>
